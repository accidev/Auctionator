local SALE_ITEM_EVENTS = {"ITEM_SEARCH_RESULTS_UPDATED", "COMMODITY_SEARCH_RESULTS_UPDATED"}
...
function AuctionatorSaleItemMixin:OnShow()
	Auctionator.EventBus:Register(self, {Auctionator.Selling.Events.BagItemClicked, Auctionator.Selling.Events.RequestPost,
                                      Auctionator.AH.Events.ThrottleUpdate, Auctionator.Selling.Events.PriceSelected,
	...
	SetOverrideBinding(self, false, Auctionator.Config.Get(Auctionator.Config.Options.SELLING_POST_SHORTCUT),
		"CLICK AuctionatorPostButton:LeftButton")
	SetOverrideBinding(self, false, Auctionator.Config.Get(Auctionator.Config.Options.SELLING_SKIP_SHORTCUT),
		"CLICK AuctionatorSkipPostingButton:LeftButton")
...
function AuctionatorSaleItemMixin:OnHide()
	Auctionator.EventBus:Unregister(self,
		{Auctionator.Selling.Events.BagItemClicked, Auctionator.Selling.Events.RequestPost,
   Auctionator.AH.Events.ThrottleUpdate, Auctionator.Selling.Events.PriceSelected,
   Auctionator.Selling.Events.RefreshSearch, Auctionator.Components.Events.EnterPressed})
...
function AuctionatorSaleItemMixin:OnUpdate()
	...
	self.TotalPrice:SetText(Auctionator.Utilities.CreateMoneyString(self.Quantity:GetNumber() * self.Price:GetAmount()))
...
function AuctionatorSaleItemMixin:GetDeposit()
	...
	if self.itemInfo.itemType == Auctionator.Constants.ITEM_TYPES.COMMODITY then
		deposit = C_AuctionHouse.CalculateCommodityDeposit(self.itemInfo.itemKey.itemID, self:GetDuration(),
			self.Quantity:GetNumber())

	elseif self.itemInfo.itemType == Auctionator.Constants.ITEM_TYPES.ITEM then
		deposit = C_AuctionHouse.CalculateItemDeposit(self.itemInfo.location, self:GetDuration(), self.Quantity:GetNumber())
	end
...
function AuctionatorSaleItemMixin:ReceiveEvent(event, ...)
	...
	elseif event == Auctionator.Selling.Events.PriceSelected and self.itemInfo ~= nil then
...
function AuctionatorSaleItemMixin:SetItemName()
	if self.itemInfo.keyName ~= nil then
		...
		Auctionator.AH.GetItemKeyInfo(self.itemInfo.itemKey, function(itemInfo)
			self.itemInfo.keyName = AuctionHouseUtil.GetItemDisplayTextFromItemKey(self.itemInfo.itemKey, itemInfo, false)
			self.TitleArea.Text:SetText(self.itemInfo.keyName)
			self:UpdateVisuals()
		end)
	end
...
function AuctionatorSaleItemMixin:UpdateForNewItem()
	...
	local price = Auctionator.Database:GetFirstPrice(Auctionator.Utilities.DBKeyFromBrowseResult({
		itemKey = self.itemInfo.itemKey
	}))
...
function AuctionatorSaleItemMixin:SetDuration()
	if Auctionator.Utilities.IsNotLIFOItemKey(self.itemInfo.itemKey) then
		self.Duration:SetSelectedValue(Auctionator.Config.Get(Auctionator.Config.Options.NOT_LIFO_AUCTION_DURATION))

	else
		self.Duration:SetSelectedValue(Auctionator.Config.Get(Auctionator.Config.Options.LIFO_AUCTION_DURATION))
	end
end
...
function AuctionatorSaleItemMixin:DoSearch(itemInfo, ...)
	...
	if IsEquipment(itemInfo) and not Auctionator.Config.Get(Auctionator.Config.Options.SELLING_GEAR_USE_ILVL) then
		...
		self.expectedItemKey = {
			itemID = itemInfo.itemKey.itemID,
			itemLevel = 0,
			itemSuffix = 0
		}
...
function AuctionatorSaleItemMixin:SetEquipmentMultiplier(itemLink)
	...
	if multiplier ~= 0 and vendorPrice ~= 0 then
		...
		self:UpdateSalesPrice(vendorPrice * multiplier + self:GetDeposit())
	end
end
...
function AuctionatorSaleItemMixin:OnEvent(eventName, ...)
	...
	elseif eventName == "ITEM_SEARCH_RESULTS_UPDATED" then
		local itemKey = ...
		if Auctionator.Utilities.ItemKeyString(itemKey) ~= Auctionator.Utilities.ItemKeyString(self.expectedItemKey) then
			return
		end
...
function AuctionatorSaleItemMixin:ProcessCommodityResults(itemID, ...)
	...
	local dbKeys = Auctionator.Utilities.DBKeyFromBrowseResult({
		itemKey = C_AuctionHouse.MakeItemKey(itemID)
	})
...
function AuctionatorSaleItemMixin:GetPostButtonState()
	return
		self.itemInfo ~= nil and self.itemInfo.count > 0 and self.itemInfo.location:IsValid() and -- Sufficient money to cover deposit
		GetMoney() > self:GetDeposit() and -- Valid quantity
		self.Quantity:GetNumber() > 0 and self.Quantity:GetNumber() <= self:GetPostLimit() and -- Positive price
		self.Price:GetAmount() > 0 and -- Bid price is not bigger than buyout
		self.BidPrice:GetAmount() <= self.Price:GetAmount() and -- Not throttled (to avoid silent post failure)
...
function AuctionatorSaleItemMixin:PostItem()
	...
	if (Auctionator.Config.Get(Auctionator.Config.Options.SELLING_AUTO_SELECT_NEXT) and
		IsValidItem(self.lastItemInfo.nextItem)) then
		-- Option to automatically select the next item in the bag view
		Auctionator.EventBus:Fire(self, Auctionator.Selling.Events.BagItemClicked, self.lastItemInfo.nextItem)
