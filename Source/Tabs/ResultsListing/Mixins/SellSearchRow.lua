...
local function BuyEntry(entry)
	...
	Auctionator.EventBus:RegisterSource(BuyEntry, "BuyEntry"):Fire(BuyEntry, Auctionator.Selling.Events.ConfirmCallback,
		entry):UnregisterSource(BuyEntry)
end
...
function AuctionatorSellSearchRowMixin:OnClick(button, ...)
	...
	if Auctionator.Utilities.IsShortcutActive(Auctionator.Config.Get(Auctionator.Config.Options.SELLING_CANCEL_SHORTCUT),
		button) then
		if C_AuctionHouse.CanCancelAuction(self.rowData.auctionID) and not self.rowData.cancelled then
			self.rowData.cancelled = true

			Auctionator.EventBus:RegisterSource(self, "SellSearchRow"):Fire(self, Auctionator.Cancelling.Events.RequestCancel,
				self.rowData.auctionID):UnregisterSource(self)
		end

	elseif Auctionator.Utilities.IsShortcutActive(Auctionator.Config.Get(Auctionator.Config.Options.SELLING_BUY_SHORTCUT),
		button) then
	...
	elseif IsModifiedClick("CHATLINK") then
		...
		Auctionator.EventBus:RegisterSource(self, "SellSearchRow"):Fire(self, Auctionator.Selling.Events.PriceSelected,
			self.rowData.price or self.rowData.bidPrice, true):UnregisterSource(self)
	end
end
