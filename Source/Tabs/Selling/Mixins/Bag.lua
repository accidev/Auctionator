...
function AuctionatorSellingBagFrameMixin:OnLoad()
	...
	self.orderedClassIds = {FAVOURITE, LE_ITEM_CLASS_WEAPON, LE_ITEM_CLASS_ARMOR, LE_ITEM_CLASS_CONTAINER,
                         LE_ITEM_CLASS_GEM, LE_ITEM_CLASS_CONSUMABLE, LE_ITEM_CLASS_GLYPH, LE_ITEM_CLASS_TRADEGOODS,
                         LE_ITEM_CLASS_RECIPE, LE_ITEM_CLASS_QUESTITEM, LE_ITEM_CLASS_AMMO, LE_ITEM_CLASS_QUIVER,
...
function AuctionatorSellingBagFrameMixin:AggregateItemsByClass()
	...
	for index = 1, bagItemCount do
		...
		if self.items[entry.classId] ~= nil then
			...
			Auctionator.Debug.Message("AuctionatorSellingBagFrameMixin:AggregateItemsByClass Missing item class table",
				entry.classId)
		end
	end
end
