...
local INCREMENTAL_SCAN_EVENTS = {"AUCTION_HOUSE_BROWSE_RESULTS_ADDED", "AUCTION_HOUSE_BROWSE_RESULTS_UPDATED",
		...
		if resultInfo.totalQuantity ~= 0 then
			...
			for index, dbKey in ipairs(allDBKeys) do
				...
				table.insert(self.info[dbKey], {
					price = resultInfo.minPrice,
					available = resultInfo.totalQuantity
				})
			end
		end
	end
