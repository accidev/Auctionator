...
function AuctionatorShoppingItemMixin:OnLoad()
	...
	self.onFinishedClicked = function()
	end
...
function AuctionatorShoppingItemMixin:OnShow()
	...
	Auctionator.EventBus:RegisterSource(self, "add item dialog"):Fire(self, Auctionator.ShoppingLists.Events.DialogOpened)
...
function AuctionatorShoppingItemMixin:OnHide()
	...
	Auctionator.EventBus:RegisterSource(self, "add item dialog"):Fire(self, Auctionator.ShoppingLists.Events.DialogClosed)
		:UnregisterSource(self)
end
...
function AuctionatorShoppingItemMixin:HasItemInfo()
	return self:GetItemString():gsub(Auctionator.Constants.AdvancedSearchDivider, ""):gsub("\"", ""):len() > 0
end
...
function AuctionatorShoppingItemMixin:GetItemString()
	...
	return searchString .. Auctionator.Constants.AdvancedSearchDivider .. self.FilterKeySelector:GetValue() ..
		       Auctionator.Constants.AdvancedSearchDivider .. self.ItemLevelRange:GetValue() ..
		       Auctionator.Constants.AdvancedSearchDivider .. self.LevelRange:GetValue() ..
		       Auctionator.Constants.AdvancedSearchDivider .. self.CraftedLevelRange:GetValue() ..
		       Auctionator.Constants.AdvancedSearchDivider .. self.PriceRange:GetValue()
end
