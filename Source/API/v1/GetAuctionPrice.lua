function Auctionator.API.v1.GetAuctionPriceByItemID(callerID, itemID)
	...
	if type(itemID) ~= "number" then
		Auctionator.API.ComposeError(callerID, "Usage Auctionator.API.v1.GetAuctionPriceByItemID(string, number)")
	end
...
function Auctionator.API.v1.GetAuctionPriceByItemLink(callerID, itemLink)
	...
	if type(itemLink) ~= "string" then
		Auctionator.API.ComposeError(callerID, "Usage Auctionator.API.v1.GetAuctionPriceByItemLink(string, string)")
	end
	...
	if dbKeys then
		...
		return Auctionator.Database:GetPrice(Auctionator.Utilities.BasicDBKeyFromLink(itemLink))
	end
end
