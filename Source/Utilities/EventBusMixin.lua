...
function AuctionatorEventBusMixin:Unregister(listener, eventNames)
	for _, eventName in ipairs(eventNames) do
		table.remove(self.registeredListeners[eventName], tIndexOf(self.registeredListeners[eventName], listener))
...
function AuctionatorEventBusMixin:Fire(source, eventName, ...)
	...
	table.insert(self.queue, {
		sourceName = self.sources[source],
		eventName = eventName,
		params = {...}
	})

	Auctionator.Debug.Message("AuctionatorEventBus:Fire()", self.sources[source], eventName, ...)
	...
	while #self.queue > 0 do
		...
		if self.registeredListeners[current.eventName] ~= nil then
			...
			local allListeners = Auctionator.Utilities.Slice(self.registeredListeners[current.eventName], 1,
