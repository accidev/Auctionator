...
function Auctionator.Variables.InitializePostingHistory()
	...
	if AUCTIONATOR_POSTING_HISTORY == nil or AUCTIONATOR_POSTING_HISTORY["__dbversion"] ~= POSTING_HISTORY_DB_VERSION then
			...
			if type(list.items) == "table" then
				for index, item in ipairs(list.items) do
					if type(item) == "string" then
						...
						if s and e then
							list.items[index] = strconcat(string.sub(item, 1, s - 1), "Изначальная тень",
								string.sub(item, e + 1))
						end
					end
				end
...
function Auctionator.Variables.InitializeShoppingLists()
	if AUCTIONATOR_SHOPPING_LISTS == nil then
		...
		if Auctionator.SavedState.ShoppingListsVersion == nil or Auctionator.SavedState.ShoppingListsVersion ~=
			SHOPPING_LISTS_VERSION then
...
function Auctionator.Variables.InitializeVendorPriceCache()
	...
	if AUCTIONATOR_VENDOR_PRICE_CACHE == nil or AUCTIONATOR_VENDOR_PRICE_CACHE["__dbversion"] ~=
		VENDOR_PRICE_CACHE_DB_VERSION then
