...
local function HideItemKey(itemKey)
	table.insert(Auctionator.Config.Get(Auctionator.Config.Options.SELLING_IGNORED_KEYS),
		Auctionator.Utilities.ItemKeyString(itemKey))

	Auctionator.EventBus:RegisterSource(HideItemKey, "HideItemKey")
		:Fire(HideItemKey, Auctionator.Selling.Events.BagRefresh):UnregisterSource(HideItemKey)
end
...
local function UnhideItemKey(itemKey)
	...
	Auctionator.EventBus:RegisterSource(UnhideItemKey, "UnhideItemKey"):Fire(UnhideItemKey,
		Auctionator.Selling.Events.BagRefresh):UnregisterSource(UnhideItemKey)
end

local function IsHidden(itemKey)
	return tIndexOf(Auctionator.Config.Get(Auctionator.Config.Options.SELLING_IGNORED_KEYS),
		Auctionator.Utilities.ItemKeyString(itemKey)) ~= nil
end
...
local function ToggleFavouriteItem(data)
	...
	Auctionator.EventBus:RegisterSource(ToggleFavouriteItem, "ToggleFavouriteItem"):Fire(ToggleFavouriteItem,
		Auctionator.Selling.Events.BagRefresh):UnregisterSource(ToggleFavouriteItem)
end

local function UnhideAllItemKeys()
	Auctionator.Config.Set(Auctionator.Config.Options.SELLING_IGNORED_KEYS, {})

	Auctionator.EventBus:RegisterSource(UnhideAllItemKeys, "UnhideAllItemKeys"):Fire(UnhideAllItemKeys,
		Auctionator.Selling.Events.BagRefresh):UnregisterSource(UnhideAllItemKeys)
end
...
function AuctionatorItemIconDropDownMixin:OnLoad()
	...
	Auctionator.EventBus:Register(self, {Auctionator.Selling.Events.ItemIconCallback})
end
