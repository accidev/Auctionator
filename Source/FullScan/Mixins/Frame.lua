...
local FULL_SCAN_EVENTS = {"REPLICATE_ITEM_LIST_UPDATE"}

local FULL_SCAN_CUSTOM_EVENTS = {"AUCTION_HOUSE_CLOSED"}
...
function AuctionatorFullScanFrameMixin:IsAutoscanReady()
	...
	return timeSinceLastScan >= (Auctionator.Config.Get(Auctionator.Config.Options.AUTOSCAN_INTERVAL) * 60) and
		       self:CanInitiate()
end

function AuctionatorFullScanFrameMixin:CanInitiate()
	return (self.state.TimeOfLastReplicateScan ~= nil and time() - self.state.TimeOfLastReplicateScan > 60 * 15 and
		       not self.inProgress) or self.state.TimeOfLastReplicateScan == nil
end
...
function AuctionatorFullScanFrameMixin:ProcessBatch(startIndex, stepSize, limit)
	...
	Auctionator.EventBus:Fire(self, Auctionator.FullScan.Events.ScanProgress, 0.2 + startIndex / limit * 0.8)
...
function AuctionatorFullScanFrameMixin:OnEvent(event, ...)
	...
	elseif event == "AUCTION_HOUSE_CLOSED" then
		...
		if self.inProgress then
			...
			Auctionator.Utilities.Message(AUCTIONATOR_L_FULL_SCAN_FAILED .. " " .. self:NextScanMessage())
