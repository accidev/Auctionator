...
function Auctionator.Search.SplitAdvancedSearch(searchString)
	local queryString, categoryKey, minItemLevel, maxItemLevel, minLevel, maxLevel, minCraftedLevel, maxCraftedLevel,
		minPrice, maxPrice = strsplit(Auctionator.Constants.AdvancedSearchDivider, searchString)
...
local function ItemLevelRange(splitSearch)
	return RangeOptionString("ilvl", splitSearch.minItemLevel, splitSearch.maxItemLevel) .. separator
end

local function CraftedLevelRange(splitSearch)
	return RangeOptionString("clvl", splitSearch.minCraftedLevel, splitSearch.maxCraftedLevel) .. separator
end

local function LevelRange(splitSearch)
	return RangeOptionString("lvl", splitSearch.minLevel, splitSearch.maxLevel) .. separator
end
...
local function PriceRange(splitSearch)
	...
	return RangeOptionString("price", min, max) .. separator
end
...
function Auctionator.Search.PrettifySearchString(searchString)
	if Auctionator.Search.IsAdvancedSearch(searchString) then
		...
		local result = splitSearch.queryString .. " [" .. CategoryKey(splitSearch) .. PriceRange(splitSearch) ..
			               LevelRange(splitSearch) .. ItemLevelRange(splitSearch) .. CraftedLevelRange(splitSearch) .. "]"
...
local function TooltipCategory(splitSearch)
	...
	return {AUCTIONATOR_L_ITEM_CLASS, key}
end

local function TooltipPriceRange(splitSearch)
	local minPrice, maxPrice = ConvertMoneyStrings(splitSearch)

	return {AUCTIONATOR_L_PRICE, TooltipRangeString(minPrice, maxPrice)}
end

local function TooltipLevelRange(splitSearch)
	return {AUCTIONATOR_L_LEVEL, TooltipRangeString(splitSearch.minLevel, splitSearch.maxLevel)}
end

local function TooltipItemLevelRange(splitSearch)
	return {AUCTIONATOR_L_ITEM_LEVEL, TooltipRangeString(splitSearch.minItemLevel, splitSearch.maxItemLevel)}
end

local function TooltipCraftedLevelRange(splitSearch)
	return {AUCTIONATOR_L_CRAFTED_LEVEL, TooltipRangeString(splitSearch.minCraftedLevel, splitSearch.maxCraftedLevel)}
end
