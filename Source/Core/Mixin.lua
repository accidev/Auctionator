...
local function InitializeIncrementalScanFrame()
	...
	if Auctionator.State.IncrementalScanFrameRef == nil then
		frame = CreateFrame("FRAME", "AuctionatorIncrementalScanFrame", AuctionHouseFrame,
	...
	if not Auctionator.Config.Get(Auctionator.Config.Options.REPLICATE_SCAN) and
		Auctionator.Config.Get(Auctionator.Config.Options.AUTOSCAN) and frame:IsAutoscanReady() then
		frame:InitiateScan()
	end
end
...
local function InitializeFullScanFrame()
	...
	if Auctionator.State.FullScanFrameRef == nil then
		frame = CreateFrame("FRAME", "AuctionatorFullScanFrame", AuctionHouseFrame, "AuctionatorFullScanFrameTemplate")
...
local function InitializeAuctionChatLogFrame()
	...
	if Auctionator.State.AuctionChatLogFrameRef == nil then
		frame = CreateFrame("FRAME", "AuctionatorAuctionChatLogFrame", AuctionHouseFrame,
...
local function InitializeAuctionHouseTabs()
	if Auctionator.State.TabFrameRef == nil then
		Auctionator.State.TabFrameRef = CreateFrame("Frame", "AuctionatorAHTabsContainer", AuctionHouseFrame,
...
local function InitializeSplashScreen()
	if Auctionator.State.SplashScreenRef == nil then
		Auctionator.State.SplashScreenRef = CreateFrame("Frame", "AuctionatorSplashScreen", UIParent,
	...
	if lastViewedSplashScreenVersion ~= mostRecentSplashScreenVersion then
		Auctionator.Config.Set(Auctionator.Config.Options.HIDE_SPLASH_SCREEN, false)
	end
