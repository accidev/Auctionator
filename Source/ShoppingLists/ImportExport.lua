...
function Auctionator.ShoppingLists.TSMImportFromString(importString)
	...
	for index, itemString in ipairs(itemStrings) do
		...
		if left == 0 then
			...
			Auctionator.EventBus:RegisterSource(Auctionator.ShoppingLists.TSMImportFromString, "TSMImportFromString"):Fire(
				Auctionator.ShoppingLists.TSMImportFromString, Auctionator.ShoppingLists.Events.ListCreated, list):UnregisterSource(
				Auctionator.ShoppingLists.TSMImportFromString)
		end
	end
end
