AuctionatorScrollListLineMixin = CreateFromMixins(ScrollListLineMixin, TableBuilderRowMixin)
...
function AuctionatorScrollListLineMixin:InitLine(currentList)
	...
	Auctionator.EventBus:Register(self,
		{Auctionator.ShoppingLists.Events.ListSelected, Auctionator.ShoppingLists.Events.ListSearchStarted,
   Auctionator.ShoppingLists.Events.ListSearchEnded, Auctionator.ShoppingLists.Events.DialogOpened,
...
local function ComposeTooltip(searchTerm)
	...
	for _, line in ipairs(tooltipDetails.lines) do
		if line[2] == AUCTIONATOR_L_ANY_LOWER then
			...
			GameTooltip:AddDoubleLine(line[1], WHITE_FONT_COLOR:WrapTextInColorCode(line[2]))
		end
	end
end
