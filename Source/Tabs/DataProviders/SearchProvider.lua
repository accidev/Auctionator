...
local SEARCH_EVENTS =
	{"COMMODITY_SEARCH_RESULTS_UPDATED", "COMMODITY_PURCHASE_SUCCEEDED", "ITEM_SEARCH_RESULTS_UPDATED"}
...
function AuctionatorSearchDataProviderMixin:OnShow()
	...
	Auctionator.EventBus:Register(self,
		{Auctionator.Selling.Events.SellSearchStart, Auctionator.Selling.Events.BagItemClicked,
...
function AuctionatorSearchDataProviderMixin:OnEvent(eventName, itemRef, auctionID)
	if eventName == "COMMODITY_SEARCH_RESULTS_UPDATED" and self.expectedItemKey ~= nil and self.expectedItemKey.itemID ==
		itemRef then
		self:Reset()
	...
	elseif (eventName == "ITEM_SEARCH_RESULTS_UPDATED" and self.expectedItemKey ~= nil and
		Auctionator.Utilities.ItemKeyString(self.expectedItemKey) == Auctionator.Utilities.ItemKeyString(itemRef)) then
...
function AuctionatorSearchDataProviderMixin:RefreshView()
	self.onPreserveScroll()
	Auctionator.EventBus:RegisterSource(self, "AuctionatorSearchDataProviderMixin"):Fire(self, Auctionator.Selling.Events
		.RefreshSearch):UnregisterSource(self)
end
...
function AuctionatorSearchDataProviderMixin:ProcessCommodityResults(itemID)
	...
	for index = C_AuctionHouse.GetNumCommoditySearchResults(itemID), 1, -1 do
		...
		if #entry.owners > 0 and #entry.owners < entry.totalNumberOfOwners then
			entry.otherSellers = AUCTIONATOR_L_SELLERS_OVERFLOW_TEXT:format(entry.otherSellers,
				entry.totalNumberOfOwners - #entry.owners)
		end
