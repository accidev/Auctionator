...
local function isDisenchantable(itemInfo)
	return #itemInfo == 0 or ((itemInfo[Auctionator.Constants.ITEM_INFO.ITEM_TYPE] == ITEM_CLASS_2 or
		       itemInfo[Auctionator.Constants.ITEM_INFO.ITEM_TYPE] == ITEM_CLASS_4) and
...
function Auctionator.Enchant.GetDisenchantReagents(itemInfo)
	...
	local deTable = classID and
		                Auctionator.Enchant.DE_TABLE[Auctionator.Enchant
			                .DE_KEY(classID, itemInfo[Auctionator.Constants.ITEM_INFO.RARITY] or 2)]
	if deTable then
		for i = 1, #deTable do
			if (itemInfo[Auctionator.Constants.ITEM_INFO.ILEVEL] >= deTable[i][1] and
				itemInfo[Auctionator.Constants.ITEM_INFO.ILEVEL] <= deTable[i][2]) then
				return deTable[i]
			end
		end
	else
