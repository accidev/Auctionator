...
function AuctionatorListRenameButtonMixin:SetUpEvents()
	...
	Auctionator.EventBus:Register(self, {ListSelected, ListCreated, RenameDialogOnAccept})
end
...
function AuctionatorListRenameButtonMixin:RenameList(newListName)
	if newListName ~= self.currentList.name then
		...
		Auctionator.ShoppingLists.Rename(Auctionator.ShoppingLists.ListIndex(self.currentList.name), newListName)
	end
