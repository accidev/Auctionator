...
function AuctionatorExportTextFrameMixin:OnShow()
	...
	if self.openEvent then
		Auctionator.EventBus:RegisterSource(self, "lists export text dialog 2"):Fire(self, self.openEvent):UnregisterSource(
			self)
	end
end
...
function AuctionatorExportTextFrameMixin:OnHide()
	...
	if self.closeEvent then
		Auctionator.EventBus:RegisterSource(self, "lists export text dialog 2"):Fire(self, self.closeEvent):UnregisterSource(
			self)
	end
end
