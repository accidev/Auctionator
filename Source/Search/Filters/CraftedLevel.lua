...
local CRAFTED_EVENTS = {Auctionator.Search.Events.BlizzardInfo}

local function IsCraftedCategory(classID)
	return classID == LE_ITEM_CLASS_GEM or classID == LE_ITEM_CLASS_ITEM_ENHANCEMENT or classID == LE_ITEM_CLASS_CONSUMABLE
end
...
function Auctionator.Search.Filters.CraftedLevelMixin:InRange(craftedLevel)
	return ( -- Minimum level check
	self.limits.min == nil or self.limits.min <= craftedLevel) and ( -- Maximum level check
	self.limits.max == nil or self.limits.max >= craftedLevel)
end
...
function Auctionator.Search.Filters.CraftedLevelMixin:ReceiveEvent(eventName, blizzardName, itemID, ...)
	...
	if (blizzardName == "GET_ITEM_INFO_RECEIVED" or blizzardName == "EXTRA_BROWSE_INFO_RECEIVED") and
