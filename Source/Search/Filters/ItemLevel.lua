...
function Auctionator.Search.Filters.ItemLevelMixin:ItemLevelCheck(itemKey)
	return ( -- Minimum item level check
	self.limits.min == nil or self.limits.min <= itemKey.itemLevel) and ( -- Maximum item level check
	self.limits.max == nil or self.limits.max >= itemKey.itemLevel)
end
