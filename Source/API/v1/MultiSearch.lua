local function ValidateState(callerID, searchTerms)
	...
	if not cloned then
		Auctionator.API.ComposeError(callerID, "Usage Auctionator.API.v1.MultiSearch(string, string[])")
	end

	for _, term in ipairs(cloned) do
		if string.match(term, "^%s*\".*\"%s*$") or string.match(term, ";") then
			Auctionator.API.ComposeError(callerID, "Search term contains ; or is wrapped in \"")
		end
	end
...
local function StartSearch(callerID, cloned)
	...
	Auctionator.EventBus:RegisterSource(StartSearch, "API v1 Multi search start"):Fire(StartSearch,
		Auctionator.ShoppingLists.Events.ListCreated, list):Fire(StartSearch,
		Auctionator.ShoppingLists.Events.ListSearchRequested, list):UnregisterSource(StartSearch)
end
